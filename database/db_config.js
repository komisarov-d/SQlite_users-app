...
const createUsersTable = (db) => {
    const usersData = JSON.parse(
        fs.readFileSync(path.resolve(__dirname, "./db_initials/users.json"), (err, data) => {
            if (err) throw err
            return data
        })
    )
    db.run(
        ...
        (err) => {
            if (err) {
                ...
                const insert = "INSERT INTO users (id, first_name, last_name, email, gender, ip_address) VALUES (?,?,?,?,?,?)"
                usersData.map((user) => {
                    db.run(insert, [
                        user.id,
                        user.first_name,
...
const createStatisticTable = (db) => {
    const statisticData = JSON.parse(
        fs.readFileSync(path.resolve(__dirname, "./db_initials/users_statistic.json"), (err, data) => {
            if (err) throw err
            return data
        })
    )
...
let db = new sqlite3.Database(path.resolve(__dirname, "./db.sqlite3"), async(err) => {
    if (err) {
